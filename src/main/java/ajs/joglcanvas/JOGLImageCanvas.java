...
public class JOGLImageCanvas extends ImageCanvas implements GLEventListener, ImageListener, KeyListener, ActionListener, ItemListener{
	...
	public JOGLImageCanvas(ImagePlus imp, boolean isMirror) {
		super(imp);
	...
	public void init(GLAutoDrawable drawable) {
		...
		rgldu=new RoiGLDrawUtility(imp, drawable,glos.getProgram("roi"));
		...
		glos.newTexture("roi-flat",2, false);
		glos.newArrayBuffer("roi-flat", avb);
		glos.newElementBuffer("roi-flat", GLBuffers.newDirectByteBuffer(eb));
		glos.newVao("roi-flat");
		glos.newFramebuffers(1, roiFramebuffers, 0);
		glos.newRenderbuffers(1, roiRenderbuffers, 0);
		
		glos.newTexture("3d-flat",imp.getNFrames()*2, false);
		glos.newArrayBuffer("3d-flat", avb);
		glos.newElementBuffer("3d-flat", GLBuffers.newDirectByteBuffer(eb));
		glos.newVao("3d-flat");
		...
		log("  Acc: "+icc.getChosenGLCapabilities());
	}
	...
	public void reshape(GLAutoDrawable drawable, int x, int y, int width, int height) {
		...
		if(JCP.debug) {
			...
			Rectangle gb=icc.getBounds();
			log("icc size  x"+gb.getX()+" y"+gb.getY()+"w"+gb.getWidth()+" h"+gb.getHeight()+" sw"+icc.getSurfaceWidth()+" sh"+icc.getSurfaceHeight());
	...
	private void resetGlobalMatrices(GLAutoDrawable drawable) {
		...
		else ratio=((double)icc.getSurfaceWidth()/icc.getSurfaceHeight())/((double)srcRect.width/srcRect.height);
		float sx=1f, sy=1f;
	...
	public void dispose(GLAutoDrawable drawable) {
		log("Disposing JOGLCanvas");
		System.out.println("Disposing JOGLCanvas -----------------------");
		glos.setGL(drawable);
		glos.dispose();
		imp.unlock();
	}
	...
	public void display(GLAutoDrawable drawable) {
		...
		if(threeDupdated || deletePBOs) {
			if(go3d) {
				...
				init3dTex();
				glos.getTexture("image3d").initiate(bitDepth, imageWidth/undersample, imageHeight/undersample, sls, 1);
				ltr=null;
			}else {
				glos.getUniformBuffer("model").loadIdentity();
		...
		if(glos.getPboLength("image")!=chs*(isFrameStack?1:frms) || deletePBOs) {
			glos.newPbo("image", chs*(isFrameStack?1:frms));
			sb.resetSlices();
			deletePBOs=false;
		}
		...
		if(myImageUpdated && !imageState.isChanged.czt && !imageState.isChanged.minmax && !scbrAdjusting.get()) {
			if(!((ij.gui.StackWindow)imp.getWindow()).getAnimate()) {
				sb.resetSlices();
				needDraw=true;
		...
		if(imageState.isChanged.minmax || imageState.isChanged.srcRect || imageState.isChanged.rotation|| cutPlanes.changed) needDraw=true;
		if(needDraw)frame3ddrawn=new boolean[frms];
		...
		double topmax=Math.pow(2, bitDepth==24?8:bitDepth)-1.0;
		for(int i=0;i<6;i++) {
			float min=0,max=0,color=0,tmin=0,tmax=0,lutType=0;
			if(luts==null || luts.length==0 ||bitDepth==24) {
				max=1f;
		...
		for(int stereoi=0;stereoi<views;stereoi++) {
			...
			//
			if(go3d) {
				glos.drawToTexture("roi-flat", stereoi, width, height, roiFramebuffers[0], roiRenderbuffers[0], bitDepth);
				glos.clearColorDepth();
			}
		...
		if(go3d) {
			...
			for(int stereoi=0;stereoi<views;stereoi++) {
				...
				glos.drawTexVao("3d-flat",fr+stereoi*frms, GL2GL3.GL_UNSIGNED_BYTE, 6, 1, false); 
				glos.drawTexVao("roi-flat",stereoi, GL2GL3.GL_UNSIGNED_BYTE, 6, 1, false); 
				glos.unBindUniformBuffer(1);
				glos.unBindUniformBuffer(2);
				glos.finish();
			}
	...
	private void createMirror() {
		...
		java.awt.EventQueue.invokeLater(new Runnable() {
			@Override
			public void run() {
				mirror.setMirrorSizeForCanvas(getWidth(), getHeight());
	...
	public boolean setFullscreen(boolean dofs) {
		if(dofs) {
			...
			if(isMirror){
				...
				fsf=new Frame();
				fsf.setLocation(imp.getWindow().getLocationOnScreen());
	...
	public void paint(Graphics g) {
		/**Only perform java painting if the canvas is a mirror
		 * (Otherwise painting is done solely in JOGL)
		 */
		if(isMirror) {
			super.paint(g);
	...
	public void addMouseListener(MouseListener object) {
		if(icc!=null && !isMirror)icc.addMouseListener(object);
		super.addMouseListener(object);
	}
	@Override
	public void addMouseMotionListener(MouseMotionListener object) {
		if(icc!=null && !isMirror)icc.addMouseMotionListener(object);
		super.addMouseMotionListener(object);
	}
	@Override
	public void addMouseWheelListener(MouseWheelListener object) {
		if(JCP.debug)IJ.log("adding mouse wheel listener to "+object+" and jea is null?"+(icc==null)+" mir"+isMirror);
		if(icc!=null && !isMirror)icc.addMouseWheelListener(object);
		super.addMouseWheelListener(object);
	}
	@Override
	public void addKeyListener(KeyListener object) {
		if(icc!=null && !isMirror)icc.addKeyListener(object);
	...
	protected void handlePopupMenu(MouseEvent e) {
		if(icc==null || e.getSource()==this)super.handlePopupMenu(e);
			...
			if (dcpopup!=null) {
				icc.add(dcpopup);
				dcpopup.show(icc,e.getX(),e.getY());
			}
		}
	}
	...
	public void createPopupMenu() {
		if(dcpopup==null) {
			...
			addCMI(menu, "Fullscreen", !(isFullscreen));
	...
	public long[] getPBOSizes() {
		return glos.getPbo("image").sizeInBytes;
	}
